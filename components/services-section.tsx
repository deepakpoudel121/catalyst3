  ...
  return (
    <section className="py-32 px-6 relative overflow-hidden" id="short-form">
      ...
      <div className="absolute inset-0 opacity-20">
        <div className="absolute top-1/2 left-1/3 w-96 h-96 bg-[#7b2cbf] rounded-full filter blur-[120px]" />
        <div className="absolute bottom-1/4 right-1/3 w-96 h-96 bg-[#1dc1cd] rounded-full filter blur-[120px]" />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service, index) => (
            <div
              key={index}
              className="group bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-8 hover:bg-gradient-to-br hover:from-[#1dc1cd]/10 hover:via-[#7b2cbf]/10 hover:to-transparent hover:border-[#1dc1cd]/30 transition-all duration-300"
            >
              <div className="w-14 h-14 bg-gradient-to-br from-[#1dc1cd] to-[#7b2cbf] rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300">
                <service.icon className="w-7 h-7 text-white" />
              </div>

              <h3 className="text-white font-bold text-xl mb-3 group-hover:text-[#1dc1cd] transition-colors">
                {service.title}
              </h3>
